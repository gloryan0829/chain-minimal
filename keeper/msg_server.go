...
// CreateGame implements checkers.MsgServer.
func (m msgServer) CreateGame(ctx context.Context, msg *checkers.MsgCreateGame) (*checkers.MsgCreateGameResponse, error) {

	if length := len([]byte(msg.Index)); checkers.MaxIndexLength < length || length < 1 {
		return nil, checkers.ErrIndexTooLong
	}

	if _, err := m.keeper.StoredGames.Get(ctx, msg.Index); err == nil {
	...
	if err := m.keeper.StoredGames.Set(ctx, msg.Index, storedGame); err != nil {
		return nil, err
	}
